...
#Purpose: This script creates the BIOM Tables ( OTU_Reads, OTU_RA, and Tax_Table)
#for input in Phyloseq# 
#Removes human and synthetic contruct reads 

###Input info from BASH####
#! /bin/Rscript 
...
{ 
  ...
  {
    if(identical(x[i,colID],sampleID[j])==TRUE){break;} 
  }
...
for( i in 1:dim(OTU_TR)[1])
{
  ...
  {
    if(is.na(OTU_TR[i,j])) OTU_TR[i,j]<-as.numeric(0)
  }
}
...
#cont_file="/usr/gapps/kpath/bin/ContFull.txt"
#C_lst<-read.table(cont_file, head=F,stringsAsFactors=F)
...
threshold=0;
if(args[2]!=0){threshold=args[2]}else{threshold=0}
spfile<-spfile[which(spfile[,2] >=threshold),]

#Create all tables
OTU_Reads<-create_BIOM(spfile,5,1,4);  Specie<-create_TAX(spfile,5,6);
OTU_RA<-OTU_Reads;
...
#Creates Taxonomic Matrix entirely
filetax="/usr/gapps/kpath/bin/Tax_Ref.2017"
...
#create_Cont(); save(OTU_Reads,OTU_Cont,Tax_Cont,Tax_Full,OTU_RA,Specie, file=paste("BIOM_",filename,".RData", sep=""))
