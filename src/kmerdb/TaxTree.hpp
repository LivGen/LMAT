...
namespace metag {
    ...
    //typedef __gnu_cxx::unordered_map<uint32_t, TaxNode*> TaxNodeHash;
    
    
    template<class tid_T>
    class TaxTree : public unordered_map<tid_T, TaxNode<tid_T>*> {
        public :
        ...
        tid_T getLcaMap(const set<tid_T> &tax_ids, unordered_map<tid_T, set<tid_T> > &children) const {
            children.clear();
