...
namespace metag {
    ...
    typedef unordered_map<uint64_t, std::pair<uint32_t, uint8_t>, hash<uint64_t>, equal_to<uint64_t>,  PERM_NS::allocator<std::pair<uint32_t, uint8_t> > > TaxTableHash;
...
#else
    typedef unordered_map<uint64_t, std::pair<uint32_t, uint8_t> > TaxTableHash;
#endif
