      ...
      if (ranks[curr_tid] == "species" and human_yes) or \
         ...
         if store_rank_val.has_key(curr_tid)   :
            ...
            for obi in range(0,num_bins,2) :
               ...
               fnd=False
               if verbose :
                  print "check tid=",tid,"ct=",curr_tid,"kc=",kmer_cnt,"pcnt=",pcnt,"no=",num_obs,lst
...
for tid in qlst :
   ...
   while True :
      ...
      if parents[curr_tid] == curr_tid : break
      curr_tid = parents[curr_tid]
   if isOther and merge_hack != [] :
      use_val = merge_hack
   if tid == 9606 and store_rank_val.has_key(9606) :
   ...
   if isEuk.has_key(tid) and use_rank == "genus" and def_euk_rval_pcnt != [] :
