...
## hard code
#bin_dir=/g/g21/allen99/LMAT-1.2.4b/bin/
...
if [ ! -e $fastsum_file ] || [ $overwrite == 1 ] ; then
   ...
   ## DEBUG -O turns of writing
   #/usr/bin/time -v $rprog -O $fstr $pstr -u $taxfile -w $rankval -x $use_min_score -j $min_read_kmer -l $hbias -b $sdiff $vstr $nullmstr -e $depthf -p -t $threads -i $query_file -d $db -c $taxtree -o $rlofile $fastqstr >& $logfile
