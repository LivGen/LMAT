...
if hash rand_read_label >& /dev/null ; then
   ...
   bin_dir=/usr/gapps/kpath/lmat/LMAT-1.2.4b/bin/
...
if [ ! $tax_histo_cnt ] ; then
   tax_histo_cnt=$LMAT_DIR/$db_file_name.kcnts
fi
...
while [ $beg -le $end ] ; do
   ...
   ${bin_dir}rand_read_label -w $rankinfo -f $conv -g $num_reads -i $read_len $vstr -e $depthf -p -t $threads -d $db_file -c $taxtree -o $ofile >& $logfile
   ...
   if [ -e $sfile ] ; then
      ...
      ${bin_dir}merge_cnts.py $sfile $taxtree $rankinfo $min_sample_size $tax_histo_cnt $odir/null.bin.$binsize.$sname $binsize
