...
export LMAT_DIR=/p/lustre1/allen99/metagenomics/runtime_inputs/04072014 #DLA:changed
...
logfile="$rlofile.log" 
tidmap="$LMAT_DIR/m9.32To16.map"  #DLA CHANGED 
...
dbssd=/l/ssd/$dbname
dbsrc=/p/lustre1/allen99/dbs/$dbname #copy of Jonathans cleaned db DLA
...
if [ ! -e $fastsum_file ] || [ $overwrite == 1 ] ; then
   ...
   echo "Process $query_file outputfile=$fastsum_file $ostr, db=$db"

   ## debug with -O option
   capture.sh $odir &
   /usr/bin/time -v $rprog $ostr $fstr $pstr -u $taxfile -w $rankval -x $use_min_score -j $min_read_kmer -l $hbias -b $sdiff $vstr $nullmstr -e $depthf -p -t $threads -i $query_file -d $db -c $taxtree -o $rlofile $fastqstr >& $logfile
   min_reads=1
