...
idfile=""
fq_file=""
prog=pull_headers.pl
usage="Retrieve headers for reads of interest and use headers to retrieve original fastq entries
Usage: $0 options

option list:
   --fq_file="" : include reads with >= this number of valid k-mers - useful when N masking is used
...
if [ -e $mergefile -a -e $fq_file ] ; then
   outfile=`echo $mergefile | sed -r 's/headers/fastq/'`
   ${bin_dir}pull_reads_fq.pl $mergefile $fq_file $outfile
fi
