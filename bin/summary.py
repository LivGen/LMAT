...
def isPlasmid(id) :
   if id >= 10000000 or id in globalPlasmids :
      return True
   return False
...
def summNode(cnode,call_type,child) :
   ...
   if (cnode in rankMap and rankMap[cnode] == call_type and not isPlasmid(cnode)) or (isPlasmid(cnode) and call_type == "plasmid") :
      ...
      while lopen != [] :
         ...
         if (call_type == "species" and not isPlasmid(alt)) or (call_type != "species" ) and rdcnt[alt] > 0 :
            ...
            tot_rdcnt += rdcnt[alt]
         if call_type == "species" and alt in rankMap and rankMap[alt] == "strain" and not isPlasmid(alt) and rdcnt[alt] > 0:
...
def bread_first_traverse(call_type,child) :
   ...
   while lopen != []:
      ...
      if (call_type == "plasmid" and isPlasmid(cnode)) or (cnode in rankMap and call_type == rankMap[cnode] and not isPlasmid(cnode)) :
...
def loadKmerStats(ifile,rank) :
   ...
   while True :
      ...
      if ln == "" or (ln.find("taxid=") != -1 and ln.find("distinct_kmer_cnt=") != -1) :
         ...
         if tid in rankMap and rank == rankMap[tid] :
...
def doPrn(save_calls,outh,names,kcov) :
   ...
   for val in rep :
      ...
      pstr += "\t"+str(rep_id)
      if rep_id in kcov :
         for kv in list(kcov[rep_id].keys()) :
