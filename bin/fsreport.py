...
def isPlasmid(plasmids,tid) :
   res=False
   if tid in plasmids or (int(tid) >= 10000000 and int(tid) < 20000000) :
      res=True
   return res
...
def getRankTid(rank,tid,ranks,parents,plasmids) :
   ...
   if (stid in ranks and rank == ranks[stid]) or (rank == "plasmid" and isPlasmid(plasmids,tid))  :
      rank_tid=tid
   else :
      while parent[stid] != stid :
         if  stid in ranks and ranks[stid] == rank :
            rank_tid=stid
            break
         stid = parent[stid]
   return rank_tid
...
for rank in list(store.keys()) :
   ...
   for tid in list(store[rank].keys()) :
      ...
      gene_lst = []
      rrna_csum=0
      if rank in gene_store and tid in gene_store[rank] :
         gene_lst=gene_store[rank][tid]
         for taxid,count in gene_lst :
            rrna_csum += int(count)
      gene_id_lst=[]
      gene_read_cnt=0
      if rank in gene_cnt and tid in gene_cnt[rank] :
         gene_id_lst=list(gene_cnt[rank][tid].keys())
